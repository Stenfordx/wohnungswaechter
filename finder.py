...
# Ganze Bezirke: werden ueber den Bezirksnamen in der Adresse erkannt.
BEZIRKE = [
...
# Einzelne Ortsteile: die Seite nennt nur den Bezirk, deshalb ueber PLZ.
PLZ = {
    # Treptow-Koepenick
    "12435": "Alt-Treptow / Plaenterwald",
    "12437": "Baumschulenweg / Plaenterwald",
    "12439": "Niederschoeneweide / Johannisthal",
    "12459": "Oberschoeneweide",
    "12487": "Johannisthal / Adlershof",
    "12489": "Adlershof",
    # Pankow
    "10119": "Prenzlauer Berg (Sued)",
...
def lage(w):
    """Gibt den Ortsteil zurueck, falls die PLZ in der Liste steht."""
...
def passt(w):
    ...
    im_ortsteil = lage(w) is not None
    if (BEZIRKE or PLZ) and not (im_bezirk or im_ortsteil):
        return False

    if w["zimmer"] is not None and not (ZIMMER_MIN <= w["zimmer"] <= ZIMMER_MAX):
        return False
    if w["flaeche"] is not None and not (FLAECHE_MIN <= w["flaeche"] <= FLAECHE_MAX):
        return False
